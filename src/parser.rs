...
pub fn parse_tables(schema: &str) -> Vec<SchemaDefinition> {
  // Updated pattern: Replaced `\|[^|]+\|` with `\|.*?\|`
  // This handles multi-argument blocks or other do-block nuances
  // so the entire table definition is captured.
  let table_re = Regex::new(r#"(?s)(create_table[\s\(]*(?:"([^"]+)"|:([a-zA-Z0-9_]+)).*?do\s*\|.*?\|.*?\bend\b)"#).expect("Invalid table regex pattern");
